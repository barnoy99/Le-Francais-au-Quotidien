...
**Current versions:** `app.js?v=95`, `style.css?v=71`, `data.js?v=40`,
`firebase-config.js?v=3`, `CACHE_VERSION = 'v88'`.
...
- **The two ⚑ passes order their sentences differently, on purpose.** ⚑ Écouter
  ...
  pair for `'rv'`, at one insertion point, and a pass laid out BEFORE pairing
  existed is repaired in place: `syncFlagQueue` re-pairs only the tail when
  `fqIsPaired()` says it is scattered, so the correction lands without losing the
  position already reached. `fqPair` sorts each pair main-first explicitly rather
  than trusting input order — it also runs over scrambled input during that
  repair. A main whose alt was already served correctly stays unpaired.
  `test/queue-test.js` asserts both rules per prefix — never "fix" one to match
  the other.
...
- **475** entries in `data.js` = **950** sentences (every entry has an alt).
